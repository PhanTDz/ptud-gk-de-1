...
def home():
    ...
    # Convert posts to list of dicts and format dates
    formatted_posts = []
    for post in posts:
        post_dict = dict(post)
...
def create_post():
    if not g.user or not session.get('is_actor'):
        return redirect(url_for('login'))
...
def my_saved_posts():
    if not g.user:
        flash('Please login to view saved posts.', 'error')
        return redirect(url_for('login'))
...
def post_detail(post_id):
    ...
    # Kiểm tra xem người dùng hiện tại có đang follow bài viết không
    is_following = False
    if g.user:
        follow = db.execute('''
            SELECT * FROM follows 
...
def delete_post(post_id):
    if 'user_id' not in session or not session.get('is_actor'):
        return redirect(url_for('home'))
...
def save_post(post_id):
    if not g.user:
        flash('Please login to save posts.', 'error')
        return redirect(url_for('login'))
    ...
    # Nếu đang ở trang saved posts thì ở lại đó, ngược lại thì về trang chi tiết bài viết
